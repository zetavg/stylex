...
export default function styleXDefineVars<Vars: VarsConfig>(
  ...
  const themeNameHash = classNamePrefix + ((options.devReadableCss || options.devReadableCssVars) ? `${options.exportName}_` : '') + createHash(themeName);
  ...
  const variablesMap = objMap(variables, (value, key) => {
    ...
    const nameHash = classNamePrefix + ((options.devReadableCss || options.devReadableCssVars) ? `${options.exportName}_${key}_` : '') + createHash(`${themeName}.${key}`);
    if (isCSSType(value)) {
