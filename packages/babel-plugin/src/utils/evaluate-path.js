...
function evaluateThemeRef(
  ...
  const resolveKey = (key: string) => {
    ...
    const varName =
      state.traversalState.options.classNamePrefix + ((state.traversalState.options.devReadableCss || state.traversalState.options.devReadableCssVars) ? `${exportName}_${key}_` : '') + utils.hash(strToHash);
    return `var(--${varName})`;
  };
